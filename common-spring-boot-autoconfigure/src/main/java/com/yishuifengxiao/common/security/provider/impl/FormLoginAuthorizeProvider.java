...
public class FormLoginAuthorizeProvider implements AuthorizeProvider {
	...
	public void config(ExpressionUrlAuthorizationConfigurer<HttpSecurity>.ExpressionInterceptUrlRegistry expressionInterceptUrlRegistry)
		...
		.formLogin()
		//权限拦截时默认跳转的页面
		.loginPage(securityProperties.getCore().getRedirectUrl())
		//处理登录请求的URL
		.loginProcessingUrl(securityProperties.getCore().getFormActionUrl())
		//用户名参数的名字
		.usernameParameter(securityProperties.getCore().getUsernameParameter())
		// 密码参数的名字
		.passwordParameter(securityProperties.getCore().getPasswordParameter())
		//自定义认证成功处理器
